...
all: jq-wasm-es6 jq-wasm-umd

.PHONY: jq-wasm-es6
jq-wasm-es6: $(JQ_VERSION)
# compile
	cd $(JQ_VERSION) && \
	  emconfigure ./configure --disable-maintainer-mode --disable-silent-rules --with-oniguruma=builtin && \
	  make clean && \
	  EMCC_CFLAGS="$(BASE_FLAGS) -s EXPORT_ES6=1" emmake make LDFLAGS=-all-static -j4
# emit output files
	mkdir -p dist && \
	  mv $(JQ_VERSION)/jq.wasm dist/jq.wasm && \
	  mv $(JQ_VERSION)/jq dist/jq.wasm.mjs

.PHONY: jq-wasm-umd
jq-wasm-umd: $(JQ_VERSION)
# compile
	cd $(JQ_VERSION) && \
	  emconfigure ./configure --disable-maintainer-mode --disable-silent-rules --with-oniguruma=builtin && \
	  make clean && \
	  EMCC_CFLAGS="$(BASE_FLAGS) -s EXPORT_ES6=0" emmake make LDFLAGS=-all-static -j4
# emit output files
	mkdir -p dist && \
